# Industry Conglomerates Tracker

Conglomerate project tracking, with the underlying data alongside the page. Single page, no build step, deployed on Netlify.
...
The figures are inline in `index.html`, with `adani_database.csv` and `adani_database_sourced.xlsx` alongside as the record. There is no API call.
